...
function TQRScanner() {
  ...
  // Handle QR scan success
  const handleScanSuccess = async (decodedText) => {
    if (attendanceMarked || attendanceProcessing) return;
    setAttendanceProcessing(true);
    ...
    try {
      ...
      console.log("Decoded QR (not JSON):", decodedText);
      qrData = { qrId: decodedText.trim() }; // fallback
    }
      ...
      async (position) => {
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            toast.error(response.data.message || "Attendance failed", { position: "top-right" });
            await stopScanner();
          }
        } catch (err) {
          console.error("Attendance API error:", err);
          toast.error(err.response?.data?.message || "Failed to mark attendance", { position: "top-right" });
          await stopScanner();
        } finally {
          setAttendanceProcessing(false);
        }
